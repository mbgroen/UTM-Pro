...
The preferences can be accessed from the iOS Settings application under "UTM Pro".

**WIP** If UTM Pro is installed using TrollStore, the preferences will be inaccessible. You can manually edit the preferences data using a 3rd party application.

## Background

### Continue running...
If enabled, UTM Pro will request location access in order to keep running in the background. If the VM uses too much memory, the operating system can still decide to terminate it.
...
If enabled, when UTM Pro enters the background, it will attempt to save the current VM state. This means that if the VM is terminated by the operating system, it can be restored in the next launch.

### Auto save on low memory
If enabled, when UTM Pro is running low on memory, it will attempt to save the current VM state. This means that if the VM is terminated by the operating system, it can be restored in the next launch.
...
For privacy reasons, you may not want UTM Pro to automatically capture a screenshot every 30 seconds and store it in the .utm package. Note that existing screenshots will not be deleted until the next time the VM is started.
