...
The URL scheme `utm` is associated with the UTM Pro app. To launch or bring to foreground UTM Pro, open the URL `utm://` (for example Safari, Shortcuts "Open URL" action, Automator).
...
| `downloadVM` | `url` to the VM, must be ZIP file of a `.utm` package (URL must contain `.zip`). | Downloads the ZIP file (progress shown in UTM Pro with cancel button), then extracts and imports the contained `.utm` file. | `utm://downloadVM?url=https%3A%2F%2Fchrisp.cafe%2FUTM%2FXP.zip` (downloads a VM template for Windows XP) |
...
## UTM Pro Automation using Shortcuts (macOS 12 Monterey and iOS)
...
## UTM Pro for Mac Automation using Automator (macOS 11 Big Sur)
