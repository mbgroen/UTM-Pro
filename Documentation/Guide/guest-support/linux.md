...
To use the experimental OpenGL hardware acceleration features in the QEMU backend, make sure a [compatible display card is selected](../settings-qemu/devices/display.md#emulated-display-card) and the VirGL driver is enabled in Mesa.

## SPICE Agent
SPICE agent is required for clipboard sharing (both [QEMU](../settings-qemu/sharing.md#clipboard-sharing) and [Apple](../settings-apple/virtualization.md#macos-13-clipboard-sharing) backend) as well as dynamic display resolution in [QEMU](../settings-qemu/devices/display.md#auto-resolution) backend.
...
## QEMU Agent
Additional features such as time syncing and [scripting](../scripting/scripting.md) are supported by the QEMU agent.
...
## SPICE WebDAV
SPICE WebDAV is required for [QEMU directory sharing](../settings-qemu/sharing.md#spice-webdav) as an alternative to VirtFS.
...
## VirtFS
VirtFS enables [QEMU directory sharing](../settings-qemu/sharing.md#virtfs) as an alternative to SPICE WebDAV. Note that VirtFS sharing must be enabled in the settings and you cannot use it at the same time as WebDAV directory sharing.
...
_Note_: `share` is the name UTM Pro uses for the VirtIO device and you should not change it. You can replace `/mnt/utm` with a different folder if you like.
...
## **macOS** VirtioFS
When using Apple Virtualization backend, [directory sharing](../settings-apple/sharing.md) is enabled through VirtioFS.
